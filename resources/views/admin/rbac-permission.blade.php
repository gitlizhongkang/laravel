    ...
    <div>
        ...
        <div class="cl pd-5 bg-1 bk-gray mt-20">
                <span class="l">
                    ...
                    <a class="btn btn-primary radius" onclick="admin_add('添加权限','{{url('/admin-rbac-addPermissionView')}}')" href="javascript:;">
                        <i class="Hui-iconfont">&#xe600;</i> 添加角色
                    </a>
                </span>
            <span class="r">共有数据：<strong>{{count($dataPermission)}}</strong> 条</span>
        </div>
            ...
            <table class="table table-border table-bordered table-bg table-hover table-sort">
                ...
                @foreach($dataPermission as $val)
                    <tr class="text-c va-m" id="{{$val->id}}">
                        ...
                        <td class="td-manage">
                            <a style="text-decoration:none" class="ml-5" onClick="admin_show('给角色绑定权限', '{{url('/admin-rbac-bindPermissionToRoleView')}}?id={{$val->id}}&name={{$val->name}}')" href="javascript:;" title="给用户绑定角色">
                                <i class="Hui-iconfont">&#xe6ff;</i>
                            </a>
